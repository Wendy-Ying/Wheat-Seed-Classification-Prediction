...
# svm
svm = SVM(kernel='linear', k=3, n_iteration=30000, lr=1e-2, early_stopping_patience=10, tol=1e-7)
svm.fit(train_features, train_labels)
test_pred = svm.get_predictions(test_features)
accuracy_score(test_labels, test_pred)
plot_confusion_matrix(test_labels, test_pred)
svm = SVM(kernel='gaussian', k=3, n_iteration=30000, lr=1e-4, early_stopping_patience=10, tol=1e-4)
svm.fit(train_features, train_labels)
test_pred = svm.get_predictions(test_features)
accuracy_score(test_labels, test_pred)
plot_confusion_matrix(test_labels, test_pred)